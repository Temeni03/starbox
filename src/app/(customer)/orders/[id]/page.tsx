  ...
  if (!order) {
    return (
      <div className="text-center py-20 text-neutral-400 pb-24 sm:pb-6">
        <p>{t('notFound')}</p>
        <Link href="/orders" className="mt-2 text-body-md text-brand-secondary hover:underline block">
          {t('backToOrders')}
        </Link>
      </div>
    )
    ...
    <div className={`space-y-4 max-w-lg mx-auto ${isAwaitingReceipt ? 'pb-40 sm:pb-28' : 'pb-24 sm:pb-6'}`}>
      ...
      {isFresh ? (
        <div className="text-center py-4">
          ...
          <h2 className="text-headline-xl text-brand-primary mb-1">{t('thankYou')}</h2>
          <p className="text-body-md text-neutral-500">{t('thankYouDesc')}</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-neutral-200 p-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-headline-md text-neutral-800">{order.orderNumber}</p>
      ...
      {isFresh && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white/70 backdrop-blur-md border border-brand-light/60 rounded-2xl p-4 flex flex-col gap-1">
            <span className="text-label-sm uppercase tracking-wider text-neutral-400">{t('orderId')}</span>
            <span className="text-headline-md text-brand-primary">{order.orderNumber}</span>
          </div>
          <div className="bg-white/70 backdrop-blur-md border border-brand-light/60 rounded-2xl p-4 flex flex-col gap-1">
            <span className="text-label-sm uppercase tracking-wider text-neutral-400">{tCommon('delivery')}</span>
      ...
      {isCancelled ? (
        <div className="bg-status-cancelled/10 border border-status-cancelled/20 rounded-2xl p-4 flex items-center gap-3">
          <Icon name="cancel" size={20} className="text-status-cancelled shrink-0" />
          <p className="text-body-md text-status-cancelled font-medium">{t('cancelled')}</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-neutral-200 p-4">
          <h2 className="text-headline-md text-neutral-800 mb-4">{t('progress')}</h2>
              ...
              return (
                <div key={step} className="relative z-10 flex-1 flex flex-col items-center gap-1.5">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition ${
                    ...
                    {done
                      ? <Icon name="check_circle" size={16} className="text-white" filled />
                      : <Icon name="schedule" size={16} className="text-neutral-400" />
                    }
                  </div>
          ...
          {order.status === 'delivered' && order.receivedAt && (
            <p className="text-label-sm text-neutral-400 text-center mt-4">
              {t('receivedOn', {
          ...
          {order.items?.map((item: any, idx: number) => (
            <div key={idx} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-label-sm text-neutral-400">× {item.quantity}</p>
              </div>
              <p className="text-body-md font-medium">{(item.price * item.quantity).toLocaleString()} MRU</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 p-4 space-y-3">
        <div className="flex items-start gap-2">
          <Icon name="location_on" size={16} className="text-neutral-400 mt-0.5 shrink-0" />
          <div>
            <p className="text-label-sm text-neutral-600">
              {order.deliveryOption === 'home' ? tCheckout('homeDelivery') : tCheckout('storePickup')}
            </p>
            {order.deliveryAddress && (
              <p className="text-label-sm text-neutral-500 mt-0.5">{order.deliveryAddress}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Icon name="credit_card" size={16} className="text-neutral-400 shrink-0" />
          <p className="text-label-sm text-neutral-600">
      ...
      <div className="bg-white/70 backdrop-blur-md border-2 border-brand-container/20 rounded-2xl p-5 space-y-3">
        <div className="flex justify-between text-body-md text-neutral-600">
        ...
        <div className="border-t border-neutral-200 pt-3 flex justify-between items-center">
          ...
          <span className="text-headline-xl text-brand-primary">{order.grandTotal?.toLocaleString()} MRU</span>
        </div>
      </div>
      ...
      {isFresh && (
        <Link
          href="/"
          className="block w-full text-center h-12 leading-12 bg-brand-primary text-white rounded-full text-label-lg shadow-lg shadow-brand-primary/20 hover:bg-brand-secondary active:scale-95 transition-all"
        >
          {t('continueShopping')}
        </Link>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl border border-neutral-200 max-w-sm w-full p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-headline-md text-neutral-800">{t('confirmReceiptTitle')}</h2>
                <p className="text-body-md text-neutral-500 mt-1">{t('confirmReceiptDesc')}</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={confirming}
                className="px-4 py-2 text-label-lg text-white bg-success rounded-lg hover:opacity-90 transition disabled:opacity-50"
              >
